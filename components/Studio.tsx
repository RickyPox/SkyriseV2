...
import studio from "@/arrays/studios";
    ...
    return (
        <div className="max-w-[1920px]">
            <div className="gridLayout grid relative">
                ...
                >
                    Want to ditch flat storytelling? We craft dynamic videos that grab attention and keep viewers engaged. Our expert team combines
                    professional filming with seamless editing to showcase your project in the best light
                </motion.h4>
            </div>
            ...
            <div className="gridLayout grid pt-[105px]">
                ...
                >
                    {studio.map((item, i) => (
                        <div key={i} className="lg:w-[48%] w-full cursor-pointer studio-item" onClick={() => openVideo(item)}>
                            <SpotlightCard className="custom-spotlight-card" spotlightColor="rgba(239, 246, 254, 0.4)">
                                <img className="aspect-video" src={`/studio/${item.img}`} alt={`video_${i + 1}`} />
                            </SpotlightCard>
                            <div className="flex flex-col justify-center items-center mt-[20px] gap-y-[5px] text-center">
                                <p className="text-[24px] lg:text-[31px] leading-[125%] tex">{item.title}</p>
            ...
            {selectedItem && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50" onClick={() => openVideo(null)}>
